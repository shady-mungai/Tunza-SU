...
const statusBadge = (status) => {
  ...
  if (status === 'Pending Review') return <Text style={styles.statusPending}>Pending Review</Text>;
  if (status === 'Resolved') return <Text style={styles.statusResolved}>Resolved</Text>;
  return null;
};

const priorityBadge = (priority) => {
  if (priority === 'High') return <Text style={styles.priorityHigh}>High</Text>;
  if (priority === 'Medium') return <Text style={styles.priorityMedium}>Medium</Text>;
...
const MyReports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:4000/userReports?user_id=${user?.id}`);
          ...
          {reports.map((report, idx) => (
            <View key={report.id || idx} style={styles.reportCard}>
              <View>
                ...
                <Text style={styles.date}>{report.created_at ? new Date(report.created_at).toLocaleDateString() : ''}</Text>
              </View>
